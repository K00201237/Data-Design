...
delimiter //
create procedure customersNewY()
...
delimiter //
create procedure customerNames(in cityName varchar(30))

begin
select customerName from customers where city = cityName;
end;


call customerNames('NYC');
call customerNames('Paris');
call customerNames('Cambridge');
call customerNames('Madrid');

-- Q4 --

delimiter //
create procedure customerNumbers(in customerNums varchar(60))

begin
select customerName, customerNumber, creditLimit from customers where customerNumber = customerNums;
end;

call customerNumbers(103);
select * from orderdetails;

-- Q5 --

delimiter //
create procedure customerOrders(in customerOrderNo varchar(60))

begin
Select quantityOrdered from orderdetails a, orders b where  a.orderNumber = b.orderNumber and b.customerNumber= customerOrderNo ;
end;

call customerOrders(103);
call customerOrders(121);
call customerOrders(124);
call customerOrders(128);

-- Q6 --

delimiter //
create procedure orderStatus(in OrdStatus varchar(60))

begin
Select quantityOrdered from orderdetails a, orders b where  a.orderNumber = b.orderNumber and b.status= OrdStatus ;
end;

call orderStatus('Shipped');
call orderStatus('Resolved');
call orderStatus('Cancelled');
call orderStatus('On Hold');


-- Q7 --

delimiter //
create procedure counts(in CountProduct varchar(60))

begin
Select count(productLine) from products where productLine = CountProduct;
end;

call counts('Motorcycles');
call counts('Classic Cars');
call counts('Ships');
call counts('Trains');
...
delimiter //
create procedure productStats(in stats varchar(60))

begin
Select avg(buyPrice) from products where buyPrice = stats;
Select min(buyPrice) as minmum from products where buyPrice = stats;
Select max(buyPrice) as max from products where buyPrice = stats;
end;
...
delimiter //
create procedure ProductLineStats(in ProductLineStats varchar(60))

begin
Select avg(buyPrice), min(buyPrice), max(buyPrice) from products where productLine = ProductLineStats;
end;

call ProductLineStats('Motorcycles');
call ProductLineStats('Classic Cars');
call ProductLineStats('Ships');
call ProductLineStats('Trains');
...
delimiter //
create procedure orderTotal(in total varchar(30))

begin
select sum(priceEach) from orderDetails where orderNumber = total;
end;
...
delimiter //
create procedure customerStatus(in cStatus varchar(60))

begin
Select count(status) from orders where status = 'Shipped' or 'Cancelled' or 'Resolved' or 'Disputed' and customerNumber = cStatus;
end;

call customerStatus(128);
...
delimiter $$
create procedure twelve(in product_line varchar(70))

begin
Select count(productName) into lt300 from products where quantityInStock <3000 and productLine =product_line;
Select count(productName) into lt300 from products where quantityInStock <3000 and productLine =product_line;
end;


-- Q13 --
-- Create a procedure that counts (sum quantity bought in this case) the number of items bought by a customer
-- (customer number is inputted, and total items is outputted by procedure.
-- call Procedure and test with customer number  103, 141, 166


select * from orderdetails;

delimiter $$
create procedure NumberItems(in totalNum varchar(30))

begin
Select count(quantityOrdered) from orderdetails a, orders b where  a.orderNumber = b.orderNumber and b.customerNumber = totalNum;
end;

call NumberItems('103');
call NumberItems('363');
call NumberItems('128');


-- Q14 --
-- create a procedure that sums the number of items bought by a customer and returns the customer rating for that customer
-- totalItems greater than 500 rating is Excellent for customer
-- totalItems between 300 and 500 rating is Good
-- totalItems less than 300 rating is Fair
-- Call procedure and test with customer number 103,141 and 166

delimiter $$
create procedure getCustomerRating(in p_customerNumber int(10), out p_customerLevel varchar(20))

begin
Declare cQuantity int;
Select sum(quantityOrdered) into cQuantity from orderdetails a, orders b where  a.orderNumber = b.orderNumber and b.customerNumber = p_customerNumber;
If cQuantity > 500 Then Set p_customerLevel = 'Excellent';
Elseif (cQuantity <=500 and cQuantity >=300) then set p_customerLevel = 'Good';
Elseif cQuantity <300 then set p_customerLevel = 'Fair';
end if;
end;

call getCustomerRating('103',@p_customerLevel);
select 103, @p_customerLevel;
call getCustomerRating('141', @p_customerLevel);
select 141, @p_customerLevel;
call getCustomerRating('166', @p_customerLevel);
select 166, @p_customerLevel;
